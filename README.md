...
- 支持在数字时钟的“岁时”设置页中添加、编辑、删除自定义事件，可设置颜色、不重复、每天、每周、每月、每年或自定义周期（每 N 天/周/月/年）。
...
事件文件使用 `schemaVersion: 2`，重复规则保存在 `recurrence` 对象中。例如下面的事件从 2026-01-05 开始每两周出现一次：
...
`type` 可取 `none`、`daily`、`weekly`、`monthly`、`yearly`、`custom`；`custom` 的 `unit` 可取 `day`、`week`、`month`、`year`。每月按起始日期的日号匹配，目标月份没有该日号时跳过；每年重复的 2 月 29 日只在闰年出现。旧版 `repeatYearly` 文件会自动兼容读取。
