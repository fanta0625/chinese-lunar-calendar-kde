...
- 支持在数字时钟的“岁时”设置页中添加、编辑、删除自定义事件，可设置名称、详情、颜色、不重复、每天、每周、每月、每年或自定义周期（每 N 天/周/月/年）。
- 鼠标悬停日期时显示完整农历日期、节气、传统节日和调休说明。
- 可在设置页开启英文日期提示；开启后，日期悬浮提示会在现有农历内容下方显示英文星期、月份和公历日期。
- 自定义事件详情显示在左侧议程条目的悬停提示中；日期格悬停提示由 Plasma 平台固定控制，不显示事件详情。
...
事件文件使用 `schemaVersion: 3`，重复规则保存在 `recurrence` 对象中，详情保存在可选的 `description` 字段中。例如下面的事件从 2026-01-05 开始每两周出现一次，并在议程条目悬停提示中显示详情：
...
`type` 可取 `none`、`daily`、`weekly`、`monthly`、`yearly`、`custom`；`custom` 的 `unit` 可取 `day`、`week`、`month`、`year`。每月按起始日期的日号匹配，目标月份没有该日号时跳过；每年重复的 2 月 29 日只在闰年出现。插件可读取 schema v1、v2 和 v3；v1 的 `repeatYearly` 文件会自动兼容读取，v2 文件没有详情时按空字符串处理。新保存的文件统一使用 schema v3。
