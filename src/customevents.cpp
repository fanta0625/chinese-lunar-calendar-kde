...
bool CustomEvents::loadFile(const QString &path, QList<CustomEvent> *events, QString *error)
{
    ...
    if (!schemaVersion.isDouble() || schemaVersion.toDouble() != SchemaVersion) {
    ...
    for (qsizetype index = 0; index < array.size(); ++index) {
        ...
        if (!idValue.isString() || !dateValue.isString() || !nameValue.isString() || !colorValue.isString() || !repeatValue.isBool()) {
        ...
        event.repeatYearly = repeatValue.toBool();
...
QByteArray CustomEvents::serializeEvents(const QList<CustomEvent> &eventsToSerialize)
{
    ...
    for (const CustomEvent &event : eventsToSerialize) {
        ...
        item.insert(QStringLiteral("repeatYearly"), event.repeatYearly);
        array.append(item);
    }
